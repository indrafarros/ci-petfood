...
<script>
    ...
    function data_table() {
        var table = $('#menu_table').DataTable({
            ...
            "ajax": {
                "url": "<?= base_url('dashboard/serverside_get_submenu') ?>",
                "dataType": "json",
                "type": "POST",
            },
            "columnDefs": [{
    ...
    $(document).ready(function() {
        $("#btnAddSubMenu").click(function(e) {
            ...
            if (submenu_name == '' || submenu_id == '' || url_name == '' || icon_sub == '') {
                ...
                $.ajax({
                    url: "<?= base_url('dashboard/addSubMenu') ?>",
                    type: 'post',
                    dataType: 'json',
                    data: {
        ...
        $(document).on('click', "#btnDeleteSubMenu", function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '<?= base_url('dashboard/deleteSubMenu') ?>',
                        type: 'post',
                        dataType: 'json',
                        data: {
                            id_menu: id,
        ...
        $(document).on('click', '#btnEditSubMenu', function(e) {
            ...
            var id = $(this).data('id');
            $.ajax({
                url: '<?= base_url('dashboard/getEditSubMenu') ?>',
                type: 'post',
                dataType: 'json',
                data: {
                    id_menu: id,
        ...
        $(document).on('click', '#btnSubmitSubMenu', function(e) {
            ...
            if (id == '' || submenu_edit_name == '' || edit_url_name == '' || edit_icon_sub == '' || edit_url_name == '') {
                ...
                $.ajax({
                    url: '<?= base_url('dashboard/submitEditSubMenu') ?>',
                    dataType: 'json',
                    type: 'post',
                    data: {
                        id_edit_submenu: id,
